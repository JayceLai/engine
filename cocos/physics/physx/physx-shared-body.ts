...
import { copyPhysXTransform, getTempTransform, PX, setMassAndUpdateInertia, physXEqualsCocosVec3, physXEqualsCocosQuat } from './export-physx';
...
export class PhysXSharedBody {
    ...
    syncPhysicsWithCheck (): void {
        const node = this.node;
        const wp = node.worldPosition;
        const wr = node.worldRotation;
        const pose = this._impl.getGlobalPose();
        const DontUpdate = physXEqualsCocosVec3(pose, wp) && physXEqualsCocosQuat(pose, wr);
        if (!DontUpdate) copyPhysXTransform(this.node, pose);
    }
